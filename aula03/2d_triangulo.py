...
def draw():
    ...
    # zoom → deslocamento no eixo Z (afasta ou aproxima da tela)

    glRotatef(r, 1, 1, 0)  
    # glRotatef(angle, axisX, axisY, axisZ)
    # r → ângulo de rotação em graus
    # 1 → rotaciona no eixo X
    # 1 → rotaciona no eixo Y
    ...
    glColor3f(0, 0, 0)  # Define a cor do triângulo como preto
    glVertex3f(0, 3, 0)  # Define o vértice superior
    glVertex3f(-2, -2, 0)  # Define o vértice inferior esquerdo
    glVertex3f(2, -2, 0)  # Define o vértice inferior direito
...
def main():
    ...
    global x, y, zoom, r, ex, ey, ez  # Declara as variáveis globais para uso dentro da função
        ...
        for event in pygame.event.get():  # Captura eventos do sistema (teclado, mouse, etc.)
            ...
            if event.type == KEYDOWN:  # Se uma tecla for pressionada
                ...
                if event.key == K_a:  # Se a tecla A for pressionada
                    x += 0.2  # Move o triângulo para a direita (controle invertido)
                if event.key == K_d:  # Se a tecla D for pressionada
                    x += -0.2  # Move o triângulo para a esquerda (controle invertido)
                ...
                if event.key == K_z:  # Zoom in (aproxima)
                    zoom += 0.2
                if event.key == K_x:  # Zoom out (afasta)
                    zoom -= 0.2
