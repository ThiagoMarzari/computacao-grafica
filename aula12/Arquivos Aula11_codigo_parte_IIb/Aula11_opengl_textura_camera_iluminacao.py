...
def init_opengl(display):
    ...
    glLightModelfv(GL_LIGHT_MODEL_AMBIENT, (0.2, 0.2, 0.2, 1.0)) # Diminuída a luz ambiente para o sombreamento aparecer
    glEnable(GL_LIGHTING) # Liga o sistema de cálculos de iluminação
    glEnable(GL_LIGHT0) # Liga a primeira fonte de luz (Luz 0)
    # A posição da luz agora será definida no loop principal para ficar fixa no mundo
    ...
    glEnable(GL_LIGHT1) # Liga uma segunda fonte de luz para iluminar os objetos por trás/baixo
    glLightfv(GL_LIGHT1, GL_DIFFUSE, (0.6, 0.6, 0.7, 1)) # Luz levemente azulada/suave
    glLightfv(GL_LIGHT1, GL_SPECULAR, (0.3, 0.3, 0.3, 1))

    glEnable(GL_NORMALIZE) # [IMPORTANTE] Corrige a iluminação de objetos que sofreram glScalef (como seus modelos)
...
def main():
    ...
    sword_tex = load_texture("OBJS/sword/Textures/Sting_Base_Color.png")
    # Carregar objetos
    scene = Wavefront('OBJS/Cat/Cat.obj', collect_faces=True, parse=True) # Carrega a geometria do gato
    tree_scene = Wavefront('OBJS/low_poly_tree/Lowpoly_tree_sample.obj', collect_faces=True, parse=True) # Carrega a árvore
    sword_scene = Wavefront('OBJS/sword/Sting-Sword-lowpoly.obj', collect_faces=True, parse=True) # Carrega a espada
    ...
    while running:
        ...
        # Define a posição da luz fixa no mundo (em cima e à frente da origem)
        glLightfv(GL_LIGHT0, GL_POSITION, (0.0, 10.0, 5.0, 1.0))
        # Segunda luz posicionada atrás e abaixo para preencher as sombras
        glLightfv(GL_LIGHT1, GL_POSITION, (0.0, -2.0, -10.0, 1.0))
        ...
        # DESENHO DO MODELO OBJ (ÁRVORE)
        glPushMatrix()
        glTranslatef(-2, -1, 0) # Coloca a árvore um pouco à esquerda
        glRotatef(-90, 1, 0, 0) # Deixa a árvore em pé
        glScalef(0.02, 0.02, 0.02) # Ajusta o tamanho da árvore para ficar proporcional
        draw_obj_model(tree_scene, tex_id) # Desenha a árvore (usando a textura 'tex_id' como base)
        glPopMatrix()

        # DESENHO DO MODELO OBJ (ESPADA)
        glPushMatrix()
        glTranslatef(1.4, 0, 0) # Coloca a espada à direita do gato
        # Rotações para deixar a espada apontando para cima (pode precisar de ajuste dependendo do eixo original)
        glRotatef(-90, 1, 0, 0) 
        glScalef(0.2, 0.2, 0.2) # A espada deve precisar ser muito bem reduzida
        draw_obj_model(sword_scene, sword_tex) # Desenha a espada com sua textura
        glPopMatrix()
