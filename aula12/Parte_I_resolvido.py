...
def main():
    ...
    tex_caixa = load_texture("texturas/caixa.jpg")    # Textura para as caixas
    tex_grama = load_texture("texturas/grama.jpg")    # Textura para o chão e folhas
    tex_parede = load_texture("texturas/parede.jpg")  # Textura para a parede/muro
    tex_madeira = load_texture("texturas/madeira.jpg") # Textura para madeira
    ...
    while running:                           # Inicia loop principal
        ...
        # [MODIFICAÇÃO] Cenário novo montado dinamicamente!
        
        # 1. CHÃO (Grama Expandida)
        glPushMatrix()                       
        ...
        # 3. UMA ÁRVORE (Tronco de madeira e folhas de grama)
        # Tronco
        glPushMatrix()
        glBindTexture(GL_TEXTURE_2D, tex_madeira)
        glTranslatef(5, -1, 5)               # Lado direito, mais ao fundo
        glScalef(0.5, 2, 0.5)                # Tronco fino e alto
        draw_textured_cube()
        glPopMatrix()
        # Folhas
        glPushMatrix()
        glBindTexture(GL_TEXTURE_2D, tex_grama)
        glTranslatef(5, 2, 5)                # No topo do tronco
        glScalef(2, 2, 2)                    # Folhas volumosas
        draw_textured_cube()
        glPopMatrix()

        # 4. CONSTRUÇÃO (Forte feito de caixas empilhadas)
        for x in [-2, 0, 2]:                 # Três colunas
            for y in [0, 2]:                 # Duas linhas de altura
                glPushMatrix()
                glBindTexture(GL_TEXTURE_2D, tex_caixa) 
                glTranslatef(x - 5, y - 1, 0) # Posicionado à esquerda do cenário
                glScalef(0.95, 0.95, 0.95)    # Pequeno espaçamento entre as caixas
                draw_textured_cube()
                glPopMatrix()
        
        # Teto da construção (Tábua de Madeira)
        glPushMatrix()
        glBindTexture(GL_TEXTURE_2D, tex_madeira)
        glTranslatef(-5, 3.2, 0)               # Acima das caixas
        glScalef(3.5, 0.2, 1.5)                # Teto plano
        draw_textured_cube()
        glPopMatrix()

        # 5. CUBO PRINCIPAL MÁGICO (Flutuando e Girando no Centro)
        angulo = pygame.time.get_ticks() * 0.05 # Calcula o ângulo baseado no tempo
        glPushMatrix()                       
        glBindTexture(GL_TEXTURE_2D, tex_caixa) 
        glTranslatef(0, 1, 2)                # Posicionado no centro
        glRotatef(angulo, 1, 1, 0)           # Fazendo o cubo girar sozinho nos eixos X e Y!
        draw_textured_cube()                 
        glPopMatrix()                        
