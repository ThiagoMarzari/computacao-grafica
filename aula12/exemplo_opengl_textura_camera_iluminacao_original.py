...
def init_opengl(display):
    ...
    glLightModelfv(GL_LIGHT_MODEL_AMBIENT, (0.5, 0.5, 0.5, 1.0))  # Luz ambiente leve (cinza)
    ...
    glLightfv(GL_LIGHT0, GL_POSITION, (2, 2, -12, 1))          # Posição da luz no espaço 3D ficou um pouco acima e à direita da câmera, e um pouco mais afastada no eixo Z para iluminar o cubo de forma mais uniforme
    glLightfv(GL_LIGHT0, GL_DIFFUSE, (0.6, 0.0, 0.0, 1))     # Componente difusa (cor) ele faz a luz ter uma cor vermelha forte nessa configuracao
    glLightfv(GL_LIGHT0, GL_SPECULAR, (0.0, 1.0, 0.0, 1))    # Componente especular (brilho)
    ...
    glMaterialf(GL_FRONT, GL_SHININESS, 20)      # Fator de brilho ele faz o material parecer mais ou menos brilhante (20 é um valor moderado)
                                                 # Valores mais altos (ex: 100) criam um brilho mais concentrado, enquanto valores baixos (ex: 5) resultam em um brilho mais difuso.
    # Define projeção de perspectiva (campo de visão, proporção, plano próximo e distante)
    glMatrixMode(GL_PROJECTION)
    glLoadIdentity()
    gluPerspective(45, (display[0] / display[1]), 0.1, 50.0) #45 graus de campo de visão, proporção baseada no tamanho da janela, plano próximo a 0.1 e plano distante a 50 unidades
...
def main():
    ...
    tex_id = load_texture("./texturas/grama.jpg")                   # Carrega textura da imagem "textura.jpg"
